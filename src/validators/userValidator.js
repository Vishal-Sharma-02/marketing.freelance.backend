...
export const validateUpdateUser = (req, res, next) => {
  ...
  if (isSubscribed !== undefined && typeof isSubscribed !== "boolean") {
    return sendError(res, "Invalid request", "isSubscribed must be a boolean", 400);
  }
...
export const validateUpdateProfile = (req, res, next) => {
  ...
  if (!fullName && !emailId && !mobile && !state) {
    return sendError(res, "Invalid request", "No fields provided to update", 400);
  }

  if (role !== undefined) {
    return sendError(res, "Invalid request", "Role can only be updated by an admin", 400);
  }

  if (isSubscribed !== undefined) {
    return sendError(res, "Invalid request", "Subscription status can only be updated by an admin", 400);
  }
